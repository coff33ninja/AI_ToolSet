...
def cmd_verify_cuda(args):
    ...
    if args.tf:
        ...
        print(
            f"TF {args.tf} needs CUDA {entry['cuda']} + cuDNN {entry['cudnn']}, "
...
def cmd_select_gpus(args):
    ...
    print(
        f"Selected {label}. CUDA_VISIBLE_DEVICES is set in this process; "
...
def cmd_frames_to_video(args):
    ...
    written = frames_to_video(
        args.frames_dir, args.out_path, fps=args.fps, ext=args.ext, size=args.size
    )
...
def cmd_make_synthetic(args):
    ...
    classes = {
        name: tuple(int(c) for c in color.split(","))
        for name, color in (pair.split(":") for pair in args.classes)
    }
    data_dir = generate_synthetic(
        classes,
        args.out_dir,
...
def cmd_split_images(args):
    ...
    rows = split_image_dataset(
        args.image_dir, args.labels_csv, args.out_dir, min_area_ratio=args.min_area_ratio
    )
...
def cmd_transcribe(args):
    ...
    if args.engine == "whisper":
        result = speech.transcribe_whisper(
            args.audio, model=args.model, language=args.language, task=args.task, gpus=gpus
        )
...
def cmd_detect(args):
    ...
    gpus = [int(x) for x in args.gpus.split(",")] if args.gpus else None
    detections, _ = detect.detect_image(args.image, weights=args.model, conf=args.conf, gpus=gpus)
    if args.json:
    ...
    if args.annotate:
        out = detect.annotate(
            args.image, args.annotate, weights=args.model, conf=args.conf, gpus=gpus
        )
...
def cmd_detect_live(args):
    ...
    index = 0
    for frame, detections, fps in detect.detect_stream(
        region, weights=args.model, conf=args.conf, gpus=gpus
    ):
        detect.draw_detections(frame, detections)
        cv2.putText(
            frame, f"{fps:.1f} fps", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2
        )
...
def cmd_record_screen(args):
    ...
    if region is None:
        ...
        if region is None:
            print("Cancelled.")
            return 1
    frames = record_screen(
        region, args.out_path, duration=args.duration, fps=args.fps, codec=args.codec
    )
...
def cmd_webcam_capture(args):
    ...
    recorded, snapshots = webcam_capture(
        args.camera,
...
def cmd_record_audio(args):
    ...
    path, seconds = record_mic(args.out, args.duration, sr=args.sr, device=args.device)
...
def cmd_benchmark(args):
    ...
    if args.image:
        rows.extend(
            benchmark_yolo(
                args.image,
                weights=args.model or "yolov8n.pt",
                iterations=args.iterations,
                gpus=gpus,
            )
        )
...
def cmd_mediapipe(args):
    ...
    for frame in webcam_stream(
        camera=args.camera, solution=args.solution, mirror=not args.no_mirror
    ):
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="python -m ai_toolset", description="Reusable AI toolkit CLI"
    )
    ...
    p.add_argument(
        "--classes", required=True, help="name:BGR,R,G,B;name:BGR,R,G,B  (e.g. npc:50,50,220)"
    )
    ...
    p.add_argument("--speaker", help="reference wav for voice cloning (>= 6 s; XTTS requires it)")
    ...
    p.add_argument("--region", help="JSON region dict {top,left,width,height}")
    p.add_argument(
        "--select-region", action="store_true", help="interactively select the region first"
    )
    ...
    p.add_argument("--region", help="JSON region dict {top,left,width,height}")
    p.add_argument(
        "--duration", type=float, default=0, help="max seconds to record (0 = until ESC)"
    )
    p.add_argument("--fps", type=float, default=20.0)
    p.add_argument("--codec", default="mp4v")
    p.set_defaults(func=cmd_record_screen)

    p = sub.add_parser("webcam-capture", help="webcam preview; r=record toggle, s=snapshot, q=quit")
    ...
    p = sub.add_parser(
        "augment", help="augment images in a directory (hflip/vflip/rot/bright/etc.)"
    )
    p.add_argument("in_dir")
    p.add_argument("out_dir", nargs="?")
    p.add_argument("--ext", default="jpg")
    p.add_argument(
        "--ops",
        nargs="+",
        help="ops to apply (default all): hflip vflip rot90 rot180 rot270 blur bright hue",
    )
    p.set_defaults(func=cmd_augment)

    p = sub.add_parser("record-audio", help="record the mic to a mono WAV (voice cloning prep)")
    p.add_argument("out", help="output wav path")
    p.add_argument("--duration", type=float, default=0, help="seconds to record (0 = until Enter)")
    p.add_argument("--sr", type=int, default=16000)
    p.add_argument("--device", type=int, help="sounddevice input index")
    p.set_defaults(func=cmd_record_audio)

    p = sub.add_parser("live-transcribe", help="stream-transcribe the mic with faster-whisper")
    p.add_argument("--duration", type=float, default=60, help="total seconds (0 = until Ctrl+C)")
    ...
    p = sub.add_parser("tts-batch", help="synthesize one wav per line of a text file")
    ...
    p.add_argument("--metadata", help="also write a Coqui metadata.csv path (e.g. metadata.csv)")
    p.add_argument("--gpus", help="comma-separated physical GPU indices")
    p.set_defaults(func=cmd_tts_batch)

    p = sub.add_parser("narrate", help="synthesize each line of a text file and play it")
    p.add_argument("text_file")
    ...
    p = sub.add_parser("benchmark", help="measure STT/YOLO latency per GPU")
    p.add_argument("--audio", help="audio file to transcribe")
    p.add_argument("--image", help="image file to run YOLO on")
    p.add_argument("--engines", nargs="+", choices=["whisper", "faster"], default=["faster"])
    p.add_argument(
        "--model",
        default=None,
        help="whisper size (default: base) or YOLO weights (default: yolov8n.pt)",
    )
    p.add_argument("--iterations", type=int, default=3)
    p.add_argument("--gpus", help="comma-separated physical GPU indices (default: all detected)")
    p.add_argument("--cpu", action="store_true", help="also benchmark CPU")
    p.set_defaults(func=cmd_benchmark)

    p = sub.add_parser("ocr", help="OCR an image or screen region (Windows.Media.Ocr)")
    ...
    p.add_argument(
        "--f0", default="rmvpe", choices=["rmvpe", "crepe", "pm"], help="pitch extraction method"
    )
    p.add_argument("--index-rate", type=float, default=0.75)
    p.add_argument("--protect", type=float, default=0.33)
    p.add_argument("--pitch", type=int, default=0, help="semitone shift, e.g. 12 = one octave up")
    p.add_argument("--gpus", help="comma-separated physical GPU indices")
    ...
    p = sub.add_parser(
        "mediapipe", help="live MediaPipe overlay (pose/hands/face/holistic/selfie); s=save, q=quit"
    )
    p.add_argument("--camera", type=int, default=0)
    p.add_argument(
        "--solution", choices=["pose", "hands", "face", "holistic", "selfie"], default="pose"
    )
    p.add_argument("--no-mirror", action="store_true")
    p.add_argument("--save-dir", default="mediapipe_frames")
    p.set_defaults(func=cmd_mediapipe)

    p = sub.add_parser("web", help="serve the AI ToolSet web dashboard (fastapi extra)")
    ...
    p = sub.add_parser("get-models", help="pre-download model weights (or show status)")
    p.add_argument(
        "--yolo",
        nargs="?",
        const="yolov8n.pt",
