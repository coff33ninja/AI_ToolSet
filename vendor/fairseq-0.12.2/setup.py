...
def do_setup(package_data):
    # ai-toolset patch: skip all C/Cython extensions. This machine has no
    # Windows SDK, so native compiles fail; RVC inference only needs
    # fairseq's pure-Python HuBERT encoder path. libbleu (BLEU scoring) and
    # the *_fast batching modules fall back to numpy implementations.
    extensions[:] = []
...
if __name__ == "__main__":
    # ai-toolset patch: no examples symlink (needs developer mode/admin on
    # Windows); example package data isn't required for RVC inference.
    package_data = {
